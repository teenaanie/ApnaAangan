...
export const metadata = {
  title: "Rate card",
  description: "What Aangan would charge once the free pilot ends. Not shown to providers.",
...
/**
 * The rate card — what Aangan WOULD charge, kept out of every provider's sight.
 *
 * Charging is switched off for the pilot (migration 0020), and while it is off
 * a provider should see no money at all: a fee they are not being charged is
 * noise at best and a reason not to sign up at worst. This page is the
 * reference for when that changes — the tiers, what falls in each, and the
 * arithmetic behind them.
 *
 * Admin only, and deliberately so. It moved here from /rates on 29 August 2026,
 * when /rates became the provider's "what you get" guide.
 */
export default async function AdminRates() {
  if (!isConfigured()) redirect("/");
  const profile = await getProfile();
  if (!profile) redirect("/auth/login?next=/admin/rates");
  if (profile.role !== "admin") redirect("/provider");

  return (
    <>
      <Nav subtitle="Rate card" />
      <Shell>
        {/* ------------------------------------------------------------ hero */}
